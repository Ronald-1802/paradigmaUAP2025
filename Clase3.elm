...
-- ===========================
-- PARTE 0: FUNCIONES PERSONALIZADAS
-- ===========================


-- 1. miMap (versión recursiva)
...
miFiltro predicado lista =
    if isEmpty lista then
        ...
        in
        if predicado x then
            x :: miFiltro predicado xs
        else
            miFiltro predicado xs
...
maximo lista =
    if isEmpty lista then
        ...
        List.foldl max (head lista) (tail lista)
...
agruparPor comparador lista =
    case lista of
        ...
        x :: xs ->
            let
                ( grupo, resto ) =
                    List.span (comparador x) xs
            in
            (x :: grupo) :: agruparPor comparador resto
